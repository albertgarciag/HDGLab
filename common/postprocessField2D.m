...
function iFig = postprocessField2D(visual,plotOpts,iFig)
...
for kComp = 1:nOfComponents
    ...
    hold on
    trisurf(visual.T, visual.X(:,1), visual.X(:,2), ...
        visual.X(:,3),  visual.U(:,iComp), ...
        'EdgeColor','none','FaceColor','interp','FaceLighting','phong','FaceAlpha',plotOpts.alphaFace)
    
    if plotOpts.fieldsWithMesh
        plot3(visual.edges.X(:,:,1), visual.edges.X(:,:,2), visual.edges.X(:,:,3), 'k')
    end
    if plotOpts.fieldsWithNodes
        plot3(visual.Xnodes(:,1), visual.Xnodes(:,2), visual.Xnodes(:,3),...
