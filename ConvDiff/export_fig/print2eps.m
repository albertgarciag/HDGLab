...
%}
    ...
    if ~isempty(font_swap)
        ...
        % Swap font names
        c = 0;
        update_font_name = zeros(1000, 1);
        for b = 1:10 % Limit number of loops to avoid infinite loop case
            for a = 1:numel(M)
                M(a) = ~isequal(get(font_handles(a), 'FontName'), fonts_new{a});
                if M(a)
                    set(font_handles(a), 'FontName', fonts_new{a});
                    c = c + 1;
                    update_font_name(c) = a;
                end
            end
            if ~any(M)
                break;
        ...
        [update_font_name, M] = unique(update_font_name(1:c));
        [M, M] = sort(M);
        update_font_name = reshape(update_font_name(M), 1, []);
        
        % Swap font sizes
        c = 0;
        update_font_size = zeros(1000, 1);
        for b = 1:10 % Limit number of loops to avoid infinite loop case
            for a = 1:numel(M)
                M(a) = ~isequal(get(font_handles(a), 'FontSize'), fonts_size(a));
                if M(a)
                    set(font_handles(a), 'FontSize', fonts_size(a));
                    c = c + 1;
                    update_font_size(c) = a;
                end
            end
            if ~any(M)
                break;
        ...
        % Compute the order to revert fonts later, without the need of a loop
        [update_font_size, M] = unique(update_font_size(1:c));
        [M, M] = sort(M);
        update_font_size = reshape(update_font_size(M), 1, []);
    end
    ...
    if ~isempty(font_swap)
        for a = update_font_name
            set(font_handles(a), 'FontName', fonts{a});
        end
        
        for a = update_font_size
            set(font_handles(a), 'FontSize', fonts_size(a));
        end
    end
