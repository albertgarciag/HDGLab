...
%        hdg_Stokes_ElementalMatricesAxi(refElem, refFace, Xe, pElem, matElem, tau, faceInfo, ctt, problemParams, isPureDirichlet, intPressure);
%
% Inputs:
%
...
function [Zul, Zpl, ZLl, zuf, zpf, zLf, zur, zpr, zLr, Ke, fe, intPressure] = ...
    hdg_Stokes_ElementalMatricesAxi(refElem, refFace, Xe, pElem, matElem, tau, faceInfo, ctt, problemParams, isPureDirichlet, isAnySlipFace, intPressure)
...
% Additional variable for axisymmetric formulation
ndofI = nOfElementNodes;
...
% Matrices for axisymmetric formulation
AII = zeros(ndofI,ndofI);
AIu = zeros(ndofI,ndofU);
...
Nw = bsxfun(@times, N, (axiCoefElem.*wXY)');
NwAxi = bsxfun(@times, N, wXY');
NwN = Nw*N';
...
%    Aup(wElem,:) = Nw*dNx(:,:,iNsd)';
    Apu(:,wElem) = dNx(:,:,iNsd)*Nw';
    if iNsd==2
%        Apu(:,wElem) = Apu(:,wElem) + 2*pi*N*NwAxi';
        AIu(:,wElem) = AIu(:,wElem) - 2*pi*sqrt(nu)*N*NwAxi';
    end
    ...
    for jNsd = 1:nsd
       ...
       if jNsd==2
           ALu(zElem,wElem) = ALu(zElem,wElem) + 2*pi*sqrt(nu)*N*NwAxi';
       end
    end
end
...
%         Arp(faceNodes) = Arp(faceNodes) + N*wXY;
        ...
        for iNsd = 1:nsd
            kNsd = nsd-iNsd;
            Auu(nNodesNsd-kNsd,nNodesNsd-kNsd) = Auu(nNodesNsd-kNsd,nNodesNsd-kNsd) + N*bsxfun(@times, N', tau(iFace)*axiCoefFace.*wXY);
            Aul(nNodesNsd-kNsd,indexFaceV(iNsd:nsd:ndofFaceUhat)) = Aul(nNodesNsd-kNsd,indexFaceV(iNsd:nsd:ndofFaceUhat)) + N*bsxfun(@times, NHat', tau(iFace)*axiCoefFace.*wXY);
            Apl(faceNodes,indexFaceV(iNsd:nsd:ndofFaceUhat)) = Apl(faceNodes,indexFaceV(iNsd:nsd:ndofFaceUhat)) + N*bsxfun(@times, NHat', axiCoefFace.*wXY.*n(:,iNsd));
            All(indexFaceV(iNsd:nsd:ndofFaceUhat),indexFaceV(iNsd:nsd:ndofFaceUhat)) = All(indexFaceV(iNsd:nsd:ndofFaceUhat),indexFaceV(iNsd:nsd:ndofFaceUhat)) - NHat*bsxfun(@times, NHat', tau(iFace)*axiCoefFace.*wXY);
            Arl(1,indexFaceV(iNsd:nsd:ndofFaceUhat)) = Arl(1,indexFaceV(iNsd:nsd:ndofFaceUhat)) + (NHat*(axiCoefFace.*wXY.*n(:,iNsd)))';
            
            if isAnySlipFace
                Alp(indexFaceV(iNsd:nsd:ndofFaceUhat),faceNodes) = Alp(indexFaceV(iNsd:nsd:ndofFaceUhat),faceNodes) + NHat*bsxfun(@times, N', axiCoefFace.*wXY.*n(:,iNsd));
                Alu(indexFaceV(iNsd:nsd:ndofFaceUhat),nNodesNsd-kNsd) = Alu(indexFaceV(iNsd:nsd:ndofFaceUhat),nNodesNsd-kNsd) + NHat*bsxfun(@times, N', tau(iFace)*axiCoefFace.*wXY);
            end
            
            for jNsd = 1:nsd
                mNsd = nsd2-(iNsd-1)*nsd-jNsd;
                ALl(nNodesNsd2-mNsd,indexFaceV(iNsd:nsd:ndofFaceUhat)) = ALl(nNodesNsd2-mNsd,indexFaceV(iNsd:nsd:ndofFaceUhat)) + N*bsxfun(@times, NHat', sqrt(nu)*axiCoefFace.*wXY.*n(:,jNsd));
                
                if isAnySlipFace
                   AlL(indexFaceV(iNsd:nsd:ndofFaceUhat),nNodesNsd2-mNsd) = AlL(indexFaceV(iNsd:nsd:ndofFaceUhat),nNodesNsd2-mNsd) + NHat*bsxfun(@times, N', sqrt(nu)*axiCoefFace.*wXY.*n(:,jNsd)); 
                end
            end
        end
...
%        Arp(faceNodes) = Arp(faceNodes) + N*wXY;
        fp(faceNodes) = fp(faceNodes) + N*(sum(uD.*n,2).*axiCoefFace.*wXY);
        fn = fn - sum(sum(uD.*n,2).*axiCoefFace.*wXY,1);
        
        for iNsd = 1:nsd
            kNsd = nsd-iNsd;
            Auu(nNodesNsd-kNsd,nNodesNsd-kNsd) = Auu(nNodesNsd-kNsd,nNodesNsd-kNsd) + N*bsxfun(@times, N', tau(iFace)*axiCoefFace.*wXY);
            fu(nNodesNsd-kNsd) = fu(nNodesNsd-kNsd) + N*(uD(:,iNsd)*tau(iFace).*axiCoefFace.*wXY);
            
            for jNsd = 1:nsd
                mNsd = nsd2-(iNsd-1)*nsd-jNsd;
                fL(nNodesNsd2-mNsd) = fL(nNodesNsd2-mNsd) + N*(sqrt(nu)*uD(:,iNsd).*n(:,jNsd).*axiCoefFace.*wXY);
            end
        end
...
%        Arp(faceNodes) = Arp(faceNodes) + N*wXY;
        ...
        for iNsd = 1:nsd
            ...
            All(indexFaceV(iNsd:nsd:ndofFaceUhat),indexFaceV(iNsd:nsd:ndofFaceUhat)) = All(indexFaceV(iNsd:nsd:ndofFaceUhat),indexFaceV(iNsd:nsd:ndofFaceUhat)) - NHat*bsxfun(@times, NHat', tau(iFace)*axiCoefFace.*wXY);
            Arl(1,indexFaceV(iNsd:nsd:ndofFaceUhat)) = Arl(1,indexFaceV(iNsd:nsd:ndofFaceUhat)) + (NHat*(axiCoefFace.*wXY.*n(:,iNsd)))';
            fl(indexFaceV(iNsd:nsd:ndofFaceUhat)) = fl(indexFaceV(iNsd:nsd:ndofFaceUhat)) - NHat*(neumannTension(:,iNsd).*axiCoefFace.*wXY);
...
%        Arp(faceNodes) = Arp(faceNodes) + N*wXY;
        ...
        for iNsd = 1:nsd
            ...
            Apl(faceNodes,indexFaceV(iNsd:nsd:ndofFaceUhat)) = Apl(faceNodes,indexFaceV(iNsd:nsd:ndofFaceUhat)) + N*bsxfun(@times, NHat', axiCoefFace.*wXY.*n(:,iNsd));
            Arl(1,indexFaceV(iNsd:nsd:ndofFaceUhat)) = Arl(1,indexFaceV(iNsd:nsd:ndofFaceUhat)) + (NHat*(axiCoefFace.*wXY.*n(:,iNsd)))';
            fl(indexFaceV(iNsd:nsd:ndofFaceUhat)) = fl(indexFaceV(iNsd:nsd:ndofFaceUhat)) + NHat*(slipForce(:,iNsd).*axiCoefFace.*wXY);
            ...
            for jNsd = 1:nsd
                ...
                pNsd = nsd-jNsd;
                Alu(indexFaceV(iNsd:nsd:ndofFaceUhat),nNodesNsd-pNsd) = Alu(indexFaceV(iNsd:nsd:ndofFaceUhat),nNodesNsd-pNsd) - NHat*bsxfun(@times, N', tau(iFace)*axiCoefFace.*wXY.*E(:,nsd*(iNsd-1)+jNsd));
                All(indexFaceV(iNsd:nsd:ndofFaceUhat),indexFaceV(jNsd:nsd:ndofFaceUhat)) = All(indexFaceV(iNsd:nsd:ndofFaceUhat),indexFaceV(jNsd:nsd:ndofFaceUhat)) + NHat*bsxfun(@times, NHat', axiCoefFace.*wXY.*(D(:,nsd*(iNsd-1)+jNsd)+tau(iFace)*E(:,nsd*(iNsd-1)+jNsd)));
                Alp(indexFaceV(iNsd:nsd:ndofFaceUhat),faceNodes) = Alp(indexFaceV(iNsd:nsd:ndofFaceUhat),faceNodes) - NHat*bsxfun(@times, N', axiCoefFace.*wXY.*n(:,jNsd).*E(:,nsd*(iNsd-1)+jNsd));
                
                for lNsd = 1:nsd
                    qNsd = nsd2-(jNsd-1)*nsd-lNsd;
                    AlL(indexFaceV(iNsd:nsd:ndofFaceUhat),nNodesNsd2-qNsd) = AlL(indexFaceV(iNsd:nsd:ndofFaceUhat),nNodesNsd2-qNsd) - NHat*bsxfun(@times, N', sqrt(nu)*axiCoefFace.*wXY.*n(:,lNsd).*E(:,nsd*(iNsd-1)+jNsd));
                end
            end
        end 
    elseif iNumF==ctt.iBC_Axi
        D = [n, problemParams.betaSlip*t];
        E = [problemParams.alphaSlip*n, t];
...
% Elemental mapping -------------------------------------------------------
Z = [ALL                 zeros(ndofL,ndofI)  ALu             zeros(ndofL,ndofP)    zeros(ndofL,1);
     zeros(ndofI,ndofL)  AII                 AIu             zeros(ndofI,ndofP)    zeros(ndofI,1);
     ALu'                AIu'                Auu             Apu'                  zeros(ndofU,1);
     zeros(ndofP,ndofL)  zeros(ndofP,ndofI)  Apu             zeros(ndofP)          Arp;
     zeros(1,ndofL)      zeros(1,ndofI)      zeros(1,ndofU)  Arp'                  0]\[ALl                    fL              zeros(ndofL,1); 
                                                                                       zeros(ndofI,ndofUhat)  zeros(ndofI,1)  zeros(ndofI,1)
...
vL = 1:ndofL;
vU = ndofL+ndofI+1:ndofL+ndofI+ndofU;
vP = ndofL+ndofI+ndofU+1:ndofL+ndofI+ndofU+ndofP;
ZLl = Z(vL,1:ndofUhat);
Zul = Z(vU,1:ndofUhat);
Zpl = Z(vP,1:ndofUhat);
zLf = Z(vL,ndofUhat+1);
